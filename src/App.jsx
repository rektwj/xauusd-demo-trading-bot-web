...
export default function App() {
  ...
  const [bridgeStatus, setBridgeStatus] = useState({ checking: false, connected: false, isDemo: false, error: null });
  const [bridgeAccount, setBridgeAccount] = useState(null);
  const [bridgePositions, setBridgePositions] = useState([]);
  const bridgeConnected = bridgeStatus.connected && bridgeStatus.isDemo;
  ...
  const checkBridge = useCallback(async () => {
    ...
    try {
      ...
      setBridgeStatus({ checking: false, connected: !!health.ok, isDemo: !!health.isDemo, error: null });
    } catch (e) {
      setBridgeStatus({ checking: false, connected: false, isDemo: false, error: e.message });
    }
...
function BrokerBridgePanel({ bridgeConfig, saveBridgeConfig, bridgeStatus, bridgeAccount, checkBridge }) {
  ...
  const mode = bridgeStatus.connected && bridgeStatus.isDemo ? "MT5 DEMO (bridge)" : "PAPER (lokal)";
  const modeColor = bridgeStatus.connected && bridgeStatus.isDemo ? "#22c55e" : "#64748b";
  ...
  return (
    <div style={styles.panel}>
      ...
      {bridgeConfig.url && (
        <Row label="Status" value={bridgeStatus.checking ? "Mengecek…" : bridgeStatus.connected ? (bridgeStatus.isDemo ? "Connected · DEMO ✓" : "Connected tapi BUKAN demo — ditolak") : "Tidak terhubung"} />
      )}
