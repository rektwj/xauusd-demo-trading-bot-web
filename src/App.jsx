...
export default function App() {
  ...
  const [rawPoints, setRawPoints] = useState(() => {
    // muat candle tersimpan dari sesi sebelumnya biar tidak mulai dari nol tiap reload
    try {
      const raw = localStorage.getItem("raw-points-cache");
      if (!raw) return [];
      const parsed = JSON.parse(raw);
      const cutoff = Date.now() - 48 * 60 * 60 * 1000; // buang yg lebih tua dari 48 jam
      return Array.isArray(parsed) ? parsed.filter((pt) => pt.t >= cutoff) : [];
    } catch {
      return [];
    }
  }); // combined intraday + live-polled points
  ...
  // persist rawPoints tiap berubah, supaya histori candle bertahan lintas reload/refresh
  useEffect(() => {
    if (!rawPoints.length) return;
    try {
      const cutoff = Date.now() - 48 * 60 * 60 * 1000;
      const trimmed = rawPoints.filter((pt) => pt.t >= cutoff);
      localStorage.setItem("raw-points-cache", JSON.stringify(trimmed));
    } catch (e) {
      console.error("gagal menyimpan cache candle", e);
    }
  }, [rawPoints]);
  ...
  const fetchIntraday = useCallback(async () => {
    try {
      ...
      if (res.ok && Array.isArray(data.points)) {
        ...
        setIntradayError(data.error || `HTTP ${res.status}`);
      }
    } catch (e) {
      // tetap pakai cache/live-polled points saja, tapi catat errornya biar kelihatan di UI
      setIntradayError(e.message || "Network error");
      console.error("fetchIntraday gagal:", e);
    }
  }, []);
  ...
  const fetchHistory = useCallback(async () => {
    try {
      ...
      // history opsional untuk timeframe 1D, tapi tetap catat errornya
      setHistoryError(e.message || "Network error");
      console.error("fetchHistory gagal:", e);
    }
  }, []);
  ...
  /* ---------- analysis engine ----------
     historyDaily = riwayat chart harian asli (real history), sudah tersedia begitu
     app dimuat -- dipakai sebagai referensi timeframe lebih tinggi, bukan candle 4H
     hasil live-poll yang butuh berjam-jam untuk terkumpul cukup. */
  const analysis = useMemo(
    () => runAnalysis(candles, candles1H, historyDaily, spot?.spot_usd_oz),
    [candles, candles1H, historyDaily, spot]
  );
  ...
  return (
    <div style={styles.app}>
      ...
      {(intradayError || historyError) && (
        <div style={{ ...styles.failsafeBanner, background: "#3a2410", borderColor: "#f97316", color: "#fdba74" }}>
          <strong>⚠ DATA HISTORIS GAGAL DIMUAT:</strong>{" "}
          {intradayError && `Intraday: ${intradayError}. `}
          {historyError && `History: ${historyError}. `}
          Bot tetap jalan pakai data live-polling + cache lokal, tapi pengisian candle akan lebih lambat.
        </div>
      )}
...
/* ============================== ANALYSIS ENGINE ============================== */
// minCandles/fast/slow/slowest bisa disesuaikan per sumber data: candle 1H yang kita
// punya dibatasi 48 jam (dari feed intraday), jadi trend 1H tidak realistis dites pakai
// ambang 60 candle (butuh 60 jam) -- itu sebabnya dulu bot "kelihatan" nunggu ngumpulin
// data tanpa akhir. Daily candles (riwayat chart asli, bukan hasil live-poll) dipakai
// sebagai referensi trend jangka lebih panjang karena datanya sudah tersedia langsung
// saat load (real history), bukan menunggu terkumpul dari polling live.
function trendOf(candles, { minCandles = 20, fast = 10, slow = 20, slowest = 50 } = {}) {
  if (candles.length < minCandles) return { dir: "Unclear", emaAligned: false, priceAboveEma200: false };
  const closes = candles.map((c) => c.close);
  const slowestPeriod = Math.min(slowest, closes.length - 1);
  const e20 = ema(closes, fast), e50 = ema(closes, slow), e200 = ema(closes, slowestPeriod);
...
function runAnalysis(candles, candles1H, historyDaily, livePriceUsd) {
  const empty = {
  ...
  const ltfTrend = trendOf(candles, { minCandles: 20, fast: 10, slow: 20, slowest: Math.min(50, candles.length - 1) });
  // 1H: feed intraday cuma nyimpen 48 jam terakhir, jadi ambang & periode EMA-nya
  // disesuaikan supaya bisa dihitung begitu 48 candle itu tersedia (bukan 60+).
  const htf1H = trendOf(candles1H, { minCandles: 24, fast: 10, slow: 20, slowest: Math.min(40, candles1H.length - 1) });
  // Daily: ini riwayat chart asli (real history dari sumber data), tersedia penuh
  // begitu app load -- dipakai sebagai "higher timeframe" pengganti candle 4H sintetis
  // yang sebelumnya nggak pernah bisa cukup datanya (maks 12 candle dari window 48 jam).
  const htfDaily = trendOf(historyDaily, { minCandles: 15, fast: 10, slow: 20, slowest: Math.min(50, historyDaily.length - 1) });
  const higherTfBullish = htf1H.dir === "Bullish" && htfDaily.dir === "Bullish";
  const higherTfBearish = htf1H.dir === "Bearish" && htfDaily.dir === "Bearish";
  ...
  // ---- checklist BUY ----
  const buyChecklist = [
    { label: "Higher timeframe (1H & Daily) bullish", pass: higherTfBullish },
  ...
  const sellChecklist = [
    { label: "Higher timeframe (1H & Daily) bearish", pass: higherTfBearish },
  ...
  // PENTING: confidence sekarang diturunkan langsung dari checklist yang SAMA dipakai untuk
  // menentukan `direction` (buyScore/sellScore dari 8 item di atas), bukan dari set bobot
  // terpisah. Sebelumnya confidence pakai kombinasi bobot lain (20/20/15/15/10/5/5/5/5) yang
  // SEMUANYA baru bisa full kalau retest S/R persis terjadi BERSAMAAN dengan higher-TF, struktur,
  // momentum, DAN EMA selaras sekaligus -- kombinasi yang nyaris tidak pernah terjadi bersamaan
  // di data real, jadi confidence hampir selalu mentok jauh di bawah 85 walau `direction` sudah
  // valid (checklist >=6/8). Itu sebabnya auto trading kelihatan "tidak pernah" buka posisi
  // meskipun data historis & trend sudah tersedia penuh. Dengan confidence = skor checklist yang
  // sama (tiap item = 12.5%), 7/8 item lolos = 87.5% (>= ambang 85%), jadi otomatis konsisten
  // dengan syarat `direction` di atas dan bisa benar-benar tercapai.
  let confidence = 0;
  ...
  if (direction) {
    const score = direction === "BUY" ? buyScore : sellScore; // 0-8, item yang sama dgn checklist di atas
    confidence = Math.round((score / buyChecklist.length) * 100);
  }

  const reasons = [];
  if (direction) {
    if (trendAligned) reasons.push("Higher timeframe (1H & Daily) selaras dengan arah sinyal");
  ...
  return {
    trend: ltfTrend.dir, htf1H: htf1H.dir, htfDaily: htfDaily.dir,
...
function ChartPanel({ candles, analysis, timeframe, setTimeframe }) {
  const interval = TV_INTERVAL_MAP[timeframe] || "15";
  const config = {
    autosize: true,
    symbol: "OANDA:XAUUSD",
    interval,
  ...
  const src = `https://s.tradingview.com/embed-widget/advanced-chart/?locale=id#${encodeURIComponent(JSON.stringify(config))}`;

  return (
    <div style={styles.panel}>
      <TfSelector timeframe={timeframe} setTimeframe={setTimeframe} />
      <div style={{ width: "100%", height: 440, borderRadius: 8, overflow: "hidden", background: "#0b0f17" }}>
        <iframe
          key={interval}
      ...
      <div style={styles.legendRow}>
        <span style={{ fontSize: 11, color: "#64748b" }}>
          Chart live dari TradingView (OANDA:XAUUSD). Level Entry/SL/TP tetap dihitung dari mesin analisis
          internal (lihat panel AI Analysis) — belum digambar otomatis di chart TradingView ini.
        </span>
      </div>
    </div>
  );
...
function AnalysisPanel({ analysis, canOpenNewTrade }) {
  ...
  return (
    <div style={styles.panel}>
      ...
      <Row label="HTF 1H / Daily" value={`${analysis.htf1H || "-"} / ${analysis.htfDaily || "-"}`} />
...
function AccountPanel({ account, eq, floatingPl, totalPl, winRate, profitFactor, closedCount, bridgeConnected, bridgeAccount }) {
  if (bridgeConnected && bridgeAccount) {
    const isReal = bridgeAccount.isDemo === false;
    return (
      <div style={styles.panel}>
        <div style={{ ...styles.sectionTitle, color: isReal ? "#f97316" : styles.sectionTitle.color }}>
          {isReal ? "MT5 REAL ACCOUNT (live dari bridge) ⚠️" : "MT5 DEMO ACCOUNT (live dari bridge)"}
        </div>
        ...
        <div style={{ fontSize: 10, color: "#64748b", marginTop: 6 }}>
          Data langsung dari akun {isReal ? "REAL" : "demo"} MT5 Anda lewat bridge, refresh tiap 15 detik.
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={styles.panel}>
      ...
      <div style={{ fontSize: 11, color: "#94a3b8", lineHeight: 1.5 }}>
        Bridge adalah server kecil (kode & panduan terpisah) yang menghubungkan lewat MetaApi.cloud ke akun MT4/MT5 Anda —
        kredensial MT5 hanya disimpan di server itu, tidak pernah di sini. Secara default bridge hanya menerima akun DEMO.
        Akun REAL hanya diterima kalau server sengaja diaktifkan lewat <code>ALLOW_REAL_TRADING=true</code>, dan tetap dibatasi
        volume maksimum per order. Selama belum tersambung, semua order tetap jalan sebagai paper trading lokal.
      </div>
    </div>
  );
}

function PositionsAndJournal({ account, spot, fxRate, onClose, bridgeConnected, bridgePositions, bridgeAccount }) {
  if (bridgeConnected) {
    const isReal = bridgeAccount?.isDemo === false;
    return (
      <div style={{ ...styles.panel, gridColumn: "1 / -1" }}>
        <div style={{ ...styles.sectionTitle, color: isReal ? "#f97316" : styles.sectionTitle.color }}>
          {isReal ? "OPEN POSITIONS (MT5 REAL — live dari bridge) ⚠️" : "OPEN POSITIONS (MT5 Demo — live dari bridge)"}
        </div>
        {bridgePositions.length === 0 ? (
          <div style={styles.emptyNote}>Tidak ada posisi terbuka di akun {isReal ? "real" : "demo"}.</div>
        ) : (
